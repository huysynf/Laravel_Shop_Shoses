$(function () {
    ...
    $('.delete-cart-item').click(function () {
        ...
        destroyResourceByAjax(url).then(data => {
            alertSuccessRe(data.message).then(data => {
                    location.reload();
            })
        })
    })
    ...
    $('.quantity-update').click(function () {
        ...
        callAjax1(url, data, 'post').then(data => {
            if(data.status == 401)
            {
                alertSuccessRe('Số lượng phải lớn hơn 1').then(data => {
                    location.reload();
                })
            }
            else {
